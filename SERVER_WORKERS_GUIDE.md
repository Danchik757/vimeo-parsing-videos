...
- `run_server.sh` — Linux server wrapper через `xvfb-run`
...
`run_server.sh` рассчитан именно на Linux и запускает coordinator через `xvfb-run`.
